...
- (NSString *)stringByEscapeWithEncoding:(CFStringEncoding)enc {
	NSString *result = self;
...
-(BOOL) isBlank
{
    return [[self stringByTrimmingLeadingAndTrailingWhitespaceAndNewlineCharacters] length ]<=0?TRUE:FALSE;
}
-(NSString *) stringByStartEnd:(NSString *)startString end:(NSString *)endString
{
    NSRange start = [self rangeOfString:startString];
    if (start.location ==NSNotFound ){
        return nil;
    }
    NSRange end = [self rangeOfString:endString options:NSLiteralSearch range:NSMakeRange(start.location, [self length]-start.location)];
    if (end.location == NSNotFound){
        return nil;
    }
    int startPosi = start.location + start.length;
    return [self substringWithRange:NSMakeRange(startPosi, end.location-startPosi)];
}
-(NSError *) errorWithCode:(int)errCode
{
    NSArray *objArray = [NSArray arrayWithObjects:self, nil, nil, nil];
...
-(NSString *) stringByReplacingMatches:(NSString *)matchRegex withTemplate:(NSString*) withTemplate
{ 
    NSError *error = NULL;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:matchRegex
